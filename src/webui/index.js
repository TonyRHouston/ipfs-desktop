...
const createWindow = () => {
  ...
  const window = new BrowserWindow({
    ...
    width: store.get(
      "window.width",
      dimensions.size.width < 1440 ? dimensions.size.width : 1440
    ),
    height: store.get(
      "window.height",
      dimensions.size.height < 900 ? dimensions.size.height : 900
    ),
    webPreferences: {
  ...
  window.webContents.once("did-start-loading", (event) => {
    ...
    window.webContents.once(
      ...
      (_, errorCode, errorDescription) => {
        webContentLoad.fail(
          new Error(`${msg}: ${errorDescription}, code: ${errorCode}`)
        );
      }
    );
  });
